...
Функция ПолучитьКэшОбъектов()
	...
	ЗаполнитьИзСправочника(КэшОбъектов, Метаданные.Справочники.Город);
	ЗаполнитьИзСправочника(КэшОбъектов, Метаданные.Справочники.Товар);
	ЗаполнитьИзСправочника(КэшОбъектов, Метаданные.Справочники.Продавец);
...
Процедура ГенерироватьНаСервере()
	...
	Для счДокументов = 1 По КоличествоДокументов Цикл
		...
		Для счСтрок = 1 По КоличествоСтрокВДокументе Цикл
			...
			Новая.Товар = Товар;
			Новая.Количество = гсч.СлучайноеЧисло(1, 100);
			Новая.Сумма = гсч.СлучайноеЧисло(1, 1000000)/10000;
		КонецЦикла;
